...
//--------------------------------------------------------------------------
{
  ...
  if (hltConfig_.init(iRun,iSetup,processName_,changed)) {
    if (changed) {
      std::cout<<"********************Module Labels**********"<<std::endl;
   // HLT_HT250_DoubleDisplacedJet60_v7 -> muones
     //HLT_HT250_v8 -> simu
     // for(int i =0; i < (int)hltConfig_.moduleLabels("HLT_HT250_DoubleDisplacedJet60_v7").size(); i++){
	  for(int i =0; i < (int)hltConfig_.moduleLabels("HLT_L2DoubleMu23_NoVertex_v9").size(); i++){
      //std::cout<<hltConfig_.moduleLabels("HLT_HT250_DoubleDisplacedJet60_v7")[i]<<std::endl;
      std::cout<<hltConfig_.moduleLabels("HLT_L2DoubleMu23_NoVertex_v9")[i]<<std::endl;
      }
//const std::vector<std::string>& HLTConfigProvider::saveTagsModules(const std::string& trigger)const;
      ...
      std::cout<<"tamano: "<< hltConfig_.streamNames().size()<<std::endl;
      for (int i = 0; i< (int)hltConfig_.streamNames().size() ; i ++){
                 
      std::cout<<hltConfig_.streamNames()[i]<<" "<<hltConfig_.streamIndex(hltConfig_.streamNames()[i])<<std::endl;}
      std::cout<<"*********************************** datasets ***************"<<std::endl;
      std::cout<<"tamano: "<<hltConfig_.streamContents().size()<<std::endl;
      for(int i = 0; i<(int)hltConfig_.streamContents().size(); i++){
         for(int j = 0; j< (int)hltConfig_.streamContent(i).size(); j ++){
       std::cout<<hltConfig_.streamContent(i)[j]<<std::endl;     
        } 
        std::cout<<"**"<<std::endl;
}
      std::cout<<"******************** names of trigger paths in dataset with index i ****************"<<std::endl;
       for (int i = 0; i<(int)hltConfig_.datasetContents().size(); i ++ ){
            std::cout<<"data set name: "<<hltConfig_.datasetName(i)<<std::endl;
           for (int j = 0; j < (int)hltConfig_.datasetContent(i).size(); j++){
     
  		std::cout<< hltConfig_.datasetContent(i)[j]<<std::endl; 
